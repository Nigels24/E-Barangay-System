...
describe("createFixedAssetAction / listFixedAssets", () => {
  it("records an asset, attributed to whoever recorded it (T-018)", async () => {
    const { db, barangay, officeEquipment, user } = await seedForWrites();

    await createFixedAssetAction(
      db,
      {
...
describe("disposeFixedAssetAction", () => {
  it("marks an asset disposed", async () => {
    const { db, barangay, officeEquipment, user } = await seedForWrites();
    const asset = await createFixedAssetAction(
      db,
      {
        barangayId: barangay.id,
        category: "Office Equipment",
    ...
    await disposeFixedAssetAction(db, { assetId: asset.id, disposalDate: "2025-06-30" }, user.id);
